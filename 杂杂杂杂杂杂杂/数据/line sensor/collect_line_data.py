#!/usr/bin/env python3
"""
Line Sensor 自动化数据收集脚本

功能:
1. 自动连接Arduino串口
2. 批量收集多个距离的line sensor数据
3. 自动整合成一个CSV文件
4. 支持断点续传

使用方法:
1. 上传 Line_only.ino 到Arduino
2. 运行: python collect_line_data.py
3. 根据提示设置好距离，启动Leader，然后按Arduino上的按钮开始采集
"""
...
# 配置
# ============================================
# 默认要测试的距离列表（cm）- 与bump sensor测试保持一致
DEFAULT_DISTANCES = [3, 5, 7, 10, 12, 15, 20, 25, 30, 35, 40]

# 注意：新版本只使用中间3个传感器(line_1, line_2, line_3)，去掉最左和最右的
...
def collect_data_for_distance(ser, distance):
    ...
    # 等待用户按按钮
    print(f"   ⚠️  请按以下步骤操作:")
    print(f"      1. 启动Leader机器人")
    print(f"      2. 将Leader放置在距离 {distance} cm 的位置")
    print(f"      3. 按Arduino上的按钮开始采集...")
...
def print_statistics(df):
    ...
    for distance in df['distance_cm'].unique():
        ...
        print(f"  center_avg 平均:  {subset['center_avg'].mean():.1f} (±{subset['center_avg'].std():.1f})")
        print(f"  center_avg 范围:  [{subset['center_avg'].min()}, {subset['center_avg'].max()}]")
        # 显示各个传感器的平均值
        if 'line_1' in subset.columns:
            print(f"  line_1 平均:      {subset['line_1'].mean():.1f}")
        if 'line_2' in subset.columns:
            print(f"  line_2 平均:      {subset['line_2'].mean():.1f}")
        if 'line_3' in subset.columns:
            print(f"  line_3 平均:      {subset['line_3'].mean():.1f}")
        print(f"  变异系数 CV:      {(subset['center_avg'].std() / subset['center_avg'].mean() * 100):.2f}%")
...
def main():
    print("\n" + "="*60)
    print("  Line Sensor 自动化数据收集工具")
    print("  (测试line sensor作为IR接收器的性能)")
    print("="*60)
    ...
    print(f"\n⚠️  重要提醒:")
    print(f"   - 需要同时使用Leader机器人作为IR发射源")
    print(f"   - 每次采集前，请确保Leader已启动并保持稳定")
    print(f"   - 测量距离时，以两个机器人正面中心为准")
