/*
 * ============================================
 * LEADER ROBOT - Bump Sensor IR方案 + 直线运动 + 数据记录
 * ============================================
 * 
 * 功能：
...
float KP_L = 0.04425f, KI_L = 0.0f, KD_L = 0.0f;  // 改：KI=0
...
// ========== 直线运动参数 ==========
#define STRAIGHT_DISTANCE_MM 400.0f  // 直线距离450mm（同直行.txt）
...
void setup() {
  ...
  Serial.println("  1. Wait for button press");
  ...
  switch (state) {
    ...
    case STATE_WAIT:
      ...
      if (digitalRead(BTN_PIN) == LOW) {
        Serial.println("Button pressed! Starting straight motion...");
        beep(100);
        delay(200);  // 防抖
        beep(100);
        ...
        if (now - last_print >= 2000) {
          last_print = now;
          Serial.println("Waiting for button press (Pin 14)...");
        }
      }
      break;
    ...
    case STATE_FINISHED:
      ...
      if (!data_printed) {
        ...
        Serial.println("\nMotion finished.");
        Serial.println("Bump Sensor IR still ON - Follower can continue to follow stopped Leader");
